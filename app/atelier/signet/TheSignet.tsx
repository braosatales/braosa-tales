...
function LanguagePicker({ selected, onChange, maxLangs }: { selected: Lang[]; onChange: (v: Lang[]) => void; maxLangs: number }) {
  ...
  return (
    <div style={{position:"relative"}}>
      ...
      <div onClick={()=>setOpen(true)} style={{background:C.t4,border:`1px solid ${open?C.purpleB:"rgba(237,224,200,0.12)"}`,borderRadius:8,padding:"12px 16px",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"space-between",minHeight:52,transition:"border-color 0.2s"}}>
      ...
      {open&&(
        <div style={{position:"absolute",top:"calc(100% + 5px)",left:0,right:0,zIndex:300,minWidth:360,background:"#1C1810",border:`1px solid ${C.purpleB}`,borderRadius:10,boxShadow:"0 20px 60px rgba(0,0,0,0.8)",overflow:"hidden"}}>
          <div style={{padding:"10px 10px 0",position:"relative"}}><input autoFocus placeholder="Search all languages…" value={search} onChange={e=>setSearch(e.target.value)} onClick={e=>e.stopPropagation()} style={{width:"100%",boxSizing:"border-box",background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:6,padding:"8px 12px",color:C.t1,fontSize:15,...GS,outline:"none"}}/><button onClick={(e)=>{ e.stopPropagation(); setOpen(false) }} style={{position:"absolute",top:10,right:10,background:"transparent",border:"none",color:"#8A7A65",cursor:"pointer",fontSize:18,lineHeight:1,padding:"4px 8px",borderRadius:4,transition:"color 0.15s"}} onMouseEnter={e=>(e.currentTarget as HTMLButtonElement).style.color="#F2E8D5"} onMouseLeave={e=>(e.currentTarget as HTMLButtonElement).style.color="#8A7A65"}>×</button></div>
          ...
          <div style={{display:"flex",maxHeight:320}}>
            {!search&&(
              <div style={{width:150,flexShrink:0,borderRight:`1px solid ${C.t4}`,overflowY:"auto",padding:"8px 0"}}>
                {Object.entries(CAT_META).map(([key,meta])=>{ const cnt=selected.filter(s=>s.category===key).length; const active=tab===key; return(
                  <div key={key} onClick={(e)=>{ e.stopPropagation(); setTab(key) }} style={{display:"flex",alignItems:"center",gap:7,padding:"7px 10px",cursor:"pointer",background:active?catBg(key,0.15):"transparent",borderRight:active?`2px solid ${catText(key)}`:"2px solid transparent",transition:"all 0.12s"}}>
            ...
            <div style={{flex:1,overflowY:"auto",padding:"6px"}}>
              ...
              {list.length===0?<div style={{color:C.t3,fontSize:15,textAlign:"center",padding:"20px 0",...GS}}>No results</div>:list.map(lang=>{ const isSel=!!selected.find(s=>s.id===lang.id); const blocked=atLimit&&!isSel; return(
                <div key={lang.id} onClick={(e)=>{ e.stopPropagation(); !blocked&&toggle(lang as Lang) }} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"9px 12px",borderRadius:6,cursor:blocked?"not-allowed":"pointer",background:isSel?catBg(lang.category,0.12):"transparent",opacity:blocked?0.35:1,transition:"background 0.1s"}} onMouseEnter={e=>{if(!isSel&&!blocked)(e.currentTarget as HTMLDivElement).style.background="rgba(255,255,255,0.04)"}} onMouseLeave={e=>{if(!isSel)(e.currentTarget as HTMLDivElement).style.background="transparent"}}>
