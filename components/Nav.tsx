  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-8 py-4 bg-brand-bg/80 backdrop-blur-sm border-b border-brand-border">
      ...
      <div className="hidden md:flex items-center gap-3">
        ...
        <SignedIn>
          <Link href="/atelier" className="font-cinzel text-xs tracking-widest uppercase text-brand-muted hover:text-brand-parchment transition-colors">
            Atelier
          </Link>
          <UserButton
            afterSignOutUrl="/"
      ...
      {menuOpen && (
        <div className="absolute top-[64px] left-0 right-0 bg-brand-bg border-b border-brand-border px-8 py-6 flex flex-col gap-5 z-50 md:hidden">
          ...
          <SignedIn>
            <Link href="/atelier" className="font-cinzel text-xs tracking-widest uppercase text-brand-muted" onClick={() => setMenuOpen(false)}>
              Atelier
            </Link>
